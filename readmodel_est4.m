function [m,p,mss] = readmodel_est4()
...
p.a1 = 0.4; % a1 varies between 0.4 (low persistence) to 0.9 (high persistence)
% passthrough of marginal costs to inflation (the impact of rmc on inflation); 
p.a2 = 0.05; % a2 varies between 0.1 (a flat Phillips curve and a high sacrifice ratio) to 0.5 (a steep Phillips curve and a low sacrifice ratio)
% the ratio of domestic costs in firms' aggregate costs
p.a3 = 0.85; % a3 varies between 0.9 (for a relatively more closed economy) to 0.5 (for a relatively more open economy)
...
% potential output with partial hysteresis
p.n1    = 0.8;%0.9283; % n1 represents the share of labor in the production function
% moving average parameter
p.u1 = 0.3789;
% okun's law parameter
p.u2 = 0.4038;

%-------- 6. Speed of convergence of selected variables to their trend values.
...
p.rho_DLA_Z_BAR   = 0.60;
p.rho_DLA_GDP_BAR = 0.70;
p.rho_RR_BAR      = 0.75;
p.rho_RR_RW_BAR   = 0.75;
...
% persistence in nairu trend growth rate
% DLA_UNEM_BAR = rho_DLA_UNEM_BAR*DLA_UNEM_BAR{-1} + SHK_DLA_UNEM_BAR;
p.rho_DLA_UNEM_BAR = 0.8801;

% persistence in unemployment trend and gap
p.rho_UNEM_BAR = 0.1097;
p.rho_UNEM_GAP = 0.4386;

%% == Standard deviations ===
p.std_SHK_L_GDP_GAP = 1.75;
p.std_SHK_L_GDP_BAR = 2.5; %*%
p.std_SHK_DLA_GDP_BAR = 0.5; %*%

p.std_SHK_DLA_CPI = 1.85;
p.std_SHK_D4L_CPI_TAR = 2.30;

p.std_SHK_L_S = 2.0;
p.std_SHK_DLA_Z_BAR = 5;

p.std_SHK_RS = 1.5;
p.std_SHK_RR_BAR = 0.3;

p.std_SHK_L_GDP_RW_GAP = 4.20;
p.std_SHK_RS_RW = 0.1;
p.std_SHK_DLA_CPI_RW = 1.9;
p.std_SHK_RR_RW_BAR = 0.08;

p.std_SHK_UNEM_BAR= 0.7;%0.8531;%0.0248;%0.1393; %*%
p.std_SHK_DLA_UNEM_BAR= 0.3119;%0.0248;%0.1393; %*%
p.std_SHK_UNEM_GAP= 0.1828;%0.4350;%2.4514; %*%
