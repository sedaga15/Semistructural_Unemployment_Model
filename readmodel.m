...
% output persistence;
p.b1 = 0.470; % b1 varies between 0.1 (extremely flexible) and 0.95(extremely persistent)

% policy passthrough (the impact of monetary policy on real economy); 
p.b2 = 0.140; % b2 varies between 0.1 (low impact) to 0.5 (strong impact)

% the impact of external demand on domestic output; 
p.b3 = 0.102; % b3 varies between 0.1 and 0.7

% the weight of the real interest rate and real exchange rate gaps in Monetary Conditions Index;
p.b4 = 0.6; % b4 varies from 0.3 to 0.8 0.5
...
p.a1 = 0.3288; % a1 varies between 0.4 (low persistence) to 0.9 (high persistence)

% passthrough of marginal costs to inflation (the impact of rmc on inflation); 
p.a2 = 0.1147; % a2 varies between 0.1 (a flat Phillips curve and a high sacrifice ratio) to 0.5 (a steep Phillips curve and a low sacrifice ratio)
...
p.rho_DLA_UNEM_BAR    = 0.9;

% Unemployment gap
% UNEM_GAP = rho_UNEM_GAP*UNEM_GAP + u2*DLA_GDP_BAR + SHK_UNEM_GAP;
p.rho_UNEM_GAP  = 0.4;
p.u2            = 0.5;

%% == Standard deviations ===
p.std_SHK_L_GDP_GAP   = 1.3;
p.std_SHK_DLA_GDP_BAR = 0.325;

p.std_SHK_DLA_CPI     = 1.3;
p.std_SHK_D4L_CPI_TAR = 1.5;

p.std_SHK_L_S = 2.55;
p.std_SHK_RS  = 0.247;

p.std_SHK_RR_BAR    = 0.01;
p.std_SHK_DLA_Z_BAR = 0.51;

p.std_SHK_L_GDP_RW_GAP = 2;
p.std_SHK_RS_RW        = 2;
p.std_SHK_DLA_CPI_RW   = 1.55;
p.std_SHK_RR_RW_BAR    = 1;

p.std_SHK_UNEM_BAR	   = 1;
p.std_SHK_DLA_UNEM_BAR	   = 1;
p.std_SHK_UNEM_GAP	   = 1;
p.std_SHK_L_GDP_BAR	   = 1;
